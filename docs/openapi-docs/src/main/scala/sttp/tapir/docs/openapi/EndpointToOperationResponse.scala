...
private[openapi] class EndpointToOperationResponse(
  ...
  private def outputsToResponse(sc: StatusCodeKey, outputs: List[EndpointOutput[_]]): Option[Response] = {
    ...
    val content = bodies
      ...
      .groupBy { case (contentType, _) => contentType }
      .mapValues(content => mergeMediaTypesToAnyOf(content.map(_._2)))
      .toListMap
  ...
  private def mergeMediaTypesToAnyOf(bodies: List[MediaType]): MediaType =
